...
==================================
Tests for make_raster_grid() — raster grid factory.
"""
...
class TestReturnType:
    ...
    def test_shape_matches(self):
        b = make_raster_grid(rows=4, cols=6)
        assert b.shape == (4, 6)
...
class TestAttrs:
    ...
    def test_attrs_independent(self):
        ...
        b = make_raster_grid(rows=3, cols=3, attrs={"a": 0, "b": 0})
        b.arrays["a"][0, 0] = 99
...
class TestEdgeCases:
    ...
    def test_1x1_grid(self):
        b = make_raster_grid(rows=1, cols=1, attrs={"state": 7})
        assert b.shape == (1, 1)
        assert b.get("state")[0, 0] == 7

    def test_1xN_grid(self):
        b = make_raster_grid(rows=1, cols=10, attrs={"state": 0})
        assert b.shape == (1, 10)
        assert b.get("state").shape == (1, 10)

    def test_Nx1_grid(self):
        b = make_raster_grid(rows=10, cols=1, attrs={"state": 0})
        assert b.shape == (10, 1)
